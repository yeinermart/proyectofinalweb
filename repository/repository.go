...
type Repository[Entity any] interface {
	// Inserta un nuevo elemento en una sola tabla.
	Create(ctx context.Context, query string, queryParams map[string]any) (int64, error)
	// Obtiene un elemento de una sola tabla
	Read(ctx context.Context, query, resourceID string) (*Entity, error)
	// Obtiene todos los elementos de una sola tabla usando límite y desplazamiento como parámetros para la paginación de resultados.
	...
	// Sobrescribe la información en una sola fila de la tabla.
...
// Recupera un objeto de persistencia personalizado para los tipos de datos específicos.
